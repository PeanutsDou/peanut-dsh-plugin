 ...
 *    EPOCH (durable event scan, resume-safe; re-injected after each
 *    compaction because the summary folds the earlier hint away) — listing
 ...
 * `prepend: true` and after `apex-bootstrap`, so it runs inside the
...
export function apply(ctx, config) {
  ...
  /**
   * Epochs that already received the hint, keyed `${sessionId}:${boundary}`.
   * PER EPOCH, not per session: a compaction folds the earlier hint into the
   * summary, so the post-compaction epoch must be re-told that instruction
   * files exist (v1.2.0 — previously once-per-session, silently lost after
   * the first compaction in long sessions).
   */
  ...
  ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
    ...
    try {
      ...
      if (hinted.has(epochKey)) return decision
      ...
      // Mark the epoch AFTER a successful probe (empty result included — the
      // probe ran, there is simply nothing to point at). On probe failure we
      // do NOT mark, so the next step retries instead of losing the hint.
      hinted.add(epochKey)
