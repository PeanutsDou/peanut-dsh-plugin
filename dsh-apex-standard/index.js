/**
 * Installer for the `apex-standard` agent preset.
 *
 * This bundle plugin copies the bundled preset
 * (preset/agent.cordis.yml, preset/preset.yml and the six plugin modules)
 * into the user preset root
 * `$DSH_HOME/.agent-presets/apex-standard/` exactly once on boot, so
 * `dsh plugin --profile web add github:rinDBeans/dsh-apex-standard`
 ...
 * `apex-standard` directory under the user preset root manually if you
 * want it gone.
 */
...
export const name = 'apex-standard-installer'

const PRESET_ID = 'apex-standard'
const PRESET_FILES = [
  'agent.cordis.yml',
  'preset.yml',
  'anchor-guardian.mjs',
