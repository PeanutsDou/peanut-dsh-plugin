...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      var ignoreUntil = 0
      var builtinToolbarEnabled = false
      ...
      function onSelection() {
        ...
        if (!builtinToolbarEnabled) { clearSettle(); return }
      ...
      function render() {
        ...
        if (ui.mode === 'actions') {
          ...
          head.addEventListener('pointerdown', function (e) {
            ...
            var startLeft = parseFloat(card.style.left) || 0
            var startTop = parseFloat(card.style.top) || 0
            var moved = false
            function onMove(ev) {
              moved = true
              var left = Math.max(0, Math.min(window.innerWidth - card.offsetWidth, startLeft + ev.clientX - startX))
              var top = Math.max(0, Math.min(window.innerHeight - card.offsetHeight, startTop + ev.clientY - startY))
            ...
            function onUp() {
              ...
              if (moved) ignoreUntil = Date.now() + 80
            }
      ...
      /** External entry used by dsh-selection-tutor: open the annotation editor
       *  for a selected passage without showing this plugin's own toolbar. */
      function startExternal(opts) {
        ...
        try {
          if (opts.range !== null && opts.range !== undefined
            && typeof opts.range.cloneRange === 'function') {
            ui.pendingAnchor = {
              range: opts.range.cloneRange(),
              seqKey: '', rowHead: '', textOffset: -1, ctxBefore: '', ctxAfter: '',
            }
          } else {
            ui.pendingAnchor = captureAnchor(text)
          }
        } catch (_) { ui.pendingAnchor = null }
        var rect = null
        try {
          if (opts.rect !== null && opts.rect !== undefined
            && typeof opts.rect.getBoundingClientRect === 'function') {
            rect = opts.rect.getBoundingClientRect()
          } else if (opts.range !== null && opts.range !== undefined
            && typeof opts.range.getBoundingClientRect === 'function') {
            rect = opts.range.getBoundingClientRect()
          }
        } catch (_) { rect = null }
        ui.pos = rect !== null && rect.width > 0 ? placeAbove(rect, 40) : { left: 8, top: 8 }
        render()
      }
      ...
      // ---------- 未发送批注的按会话持久化 ----------
      function annotationStorageKey(sessionId) {
        return 'dsh-annotation:quotes:' + sessionId
      }
      function persistQuotes(sessionId) {
        if (sessionId === undefined || sessionId === null) return
        try {
          var data = ui.quotes.map(function (q) {
            return {
              id: q.id,
              text: q.text,
              note: q.note || "",
              seqKey: q.seqKey || "",
              rowHead: q.rowHead || "",
              textOffset: typeof q.textOffset === 'number' ? q.textOffset : -1,
              ctxBefore: q.ctxBefore || "",
              ctxAfter: q.ctxAfter || "",
            }
          })
          window.localStorage.setItem(annotationStorageKey(sessionId), JSON.stringify(data))
        } catch (_) { /* storage unavailable */ }
      }
      function loadQuotes(sessionId) {
        if (sessionId === undefined || sessionId === null) return
        try {
          var raw = window.localStorage.getItem(annotationStorageKey(sessionId))
          if (raw === null) return
          var arr = JSON.parse(raw)
          if (!Array.isArray(arr)) return
          ui.quotes = arr.map(function (item) {
            return {
              id: item.id,
              text: item.text,
              note: item.note || "",
              ...
              textOffset: typeof item.textOffset === 'number' ? item.textOffset : -1,
              ctxBefore: item.ctxBefore || "",
              ctxAfter: item.ctxAfter || "",
            }
          })
        } catch (_) { ui.quotes = [] }
      }
        ...
        if (ui.editingId !== null) {
          for (var ei = 0; ei < ui.quotes.length; ei++) {
            if (ui.quotes[ei].id === ui.editingId) {
              ...
              updateChip()
              renderMarkers()
              persistQuotes(sessions.list.getSnapshot().current)
              focusComposer()
              return
            }
          }
      ...
      var unsub = sessions.list.subscribe(function () {
        ...
        lastSessionId = cur
        if (lastSessionId !== undefined) persistQuotes(lastSessionId)
        if (ui.mode !== 'closed') closeToolbar()
        ui.quotes = []
        annotationAttached = false
        ui.noteDraft = ''
      ...
      watchInputDraft()
      kickDecorate()
      loadQuotes(sessions.list.getSnapshot().current)
      updateChip()

      // Expose a minimal external API for dsh-selection-tutor integration.
      window.dshAnnotation = { start: startExternal }
      // ---------- 清理 ----------
      return function () {
        clearSettle()
